...
function ClientForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append portfolio links individually
    formData.portfolioLinks.forEach((link, index) => {
        data.append(`portfolioLinks[${index}]`, link);
    });
    ...
    try {
        const response = await axios.post("http://localhost:3000/createProfile", data, {
              ...
              <div className="mb-4">
                ...
                {formData.skills.map((skill, index) => (
                  <div key={index} className="flex gap-2 mt-2">
                    <input type="text" placeholder="Enter Skill" value={skill} onChange={(e) => handleSkillChange(index, e.target.value)} className="w-full border-2 border-black outline-0 px-5 py-2 rounded-xl" />
                  </div>
                ))}
                <button type="button" onClick={addSkillField} className="mt-2 text-blue-500">+ Add Skill</button>
              </div>
