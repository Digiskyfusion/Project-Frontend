...
const SkillPage = () => {
  ...
  const loggedInUserRole =
    JSON.parse(localStorage.getItem("user"))?.roleType || "guest"; // Ensure roleType is accessed properly
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {[...users].reverse().map((user, index) => (
            <motion.div
              ...
              whileHover={{
                scale: 1.05,
                boxShadow: "0px 5px 15px rgba(0,255,159,0.4)",
              }}
